...
public class Uattack extends JFrame implements MouseListener{
	public static ArrayList<JButton> buttons;
	
	public Uattack(ArrayList<UltimateAttack> x) {
		super();
		setVisible(true);
		setBounds(800, 200, 450, 150);
		...
		if ( x != null && x.size() > 0) {
			...
			for (UltimateAttack attack : x) {
				...
				tmp.setActionCommand("fu");
				buttons.add(tmp);
			}
			
		} else {
			buttons = new ArrayList<>();
			JButton tmp = new JButton("No Ultimate Attack");
			add(tmp);
			tmp.setFont(new Font(Font.MONOSPACED, Font.BOLD, 16));
			tmp.addMouseListener(this);
